...
const AsteroidBelt = ({ ringState, massiveAsteroidsRef }: { ringState: 'hidden' | 'animating' | 'visible', massiveAsteroidsRef: React.MutableRefObject<Float32Array> }) => {
  ...
  const explosionRef = useRef({ radius: 0.0, velocity: 8.0 });
  ...
  useFrame((state, delta) => {
    ...
    if (scaleRef.current < 0.01) {
      ...
      explosionRef.current.radius = 0.0;
      explosionRef.current.velocity = 8.0;
      return;
    }
    meshRef.current.visible = true;

    // Spring physics for explosion effect
    const tension = 60;
    const friction = 10;
    const displacement = explosionRef.current.radius - 1.0;
    const springForce = -tension * displacement;
    const dampingForce = -friction * explosionRef.current.velocity;
    explosionRef.current.velocity += (springForce + dampingForce) * delta;
    explosionRef.current.radius += explosionRef.current.velocity * delta;
    ...
    asteroids.forEach((ast, i) => {
      ...
      // Apply explosion radius multiplier
      let targetRadius = ast.baseRadius + Math.sin(ast.phase) * ast.radialAmplitude;
      let currentRadius = targetRadius * explosionRef.current.radius;
      ...
      const currentZOffset = ast.zOffset * explosionRef.current.radius;

      massiveAsteroidsRef.current[i * 4] = x;
      massiveAsteroidsRef.current[i * 4 + 1] = y;
      massiveAsteroidsRef.current[i * 4 + 2] = currentZOffset;
      ...
      dummy.position.set(x, y, currentZOffset);
      ...
      <div className="relative md:absolute md:right-0 md:top-0 w-full h-[450px] md:h-full md:w-[60%] pointer-events-auto z-0 flex items-center justify-center">
        <div className="absolute inset-0 w-full h-full">
          <Canvas shadows camera={{ position: [0, 4, 10], fov: 45 }} dpr={[1, 2]}>
            ...
            {/* Moon is kept upright (no group rotation) so poles look correct */}
            <Suspense fallback={null}>
              <RealisticMoon onClick={() => { if(ringState === 'hidden') setRingState('animating') }} />
              
              {/* 
                Outer group: Tilts the entire swoop diagonally across the screen (bottom-left to top-right).
                Inner group: Tips the vertical ring backwards by 72 degrees to create the thin edge-on swoop.
              */}
